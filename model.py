...
class SocialLSTM(nn.Module):
    ...
    def getSocialTensor(self, grid, hidden_states):
        ...
        social_tensor = Variable(torch.zeros(numNodes, self.grid_size*self.grid_size, self.rnn_size).to(torch.device("cpu")))
        # For each ped
    ...
    def forward(self, nodes, grids, nodesPresent, hidden_states, cell_states):
        ...
        outputs = Variable(torch.zeros(self.seq_length * numNodes, self.output_size)).to(torch.device("cpu"))
        ...
        for framenum in range(self.seq_length):
            ...
            # List of nodes
            list_of_nodes = Variable(torch.LongTensor(nodeIDs).to(torch.device("cpu")))
        ...
        # Reshape outputs
        outputs_return = Variable(torch.zeros(self.seq_length, numNodes, self.output_size).to(torch.device("cpu")))
        for framenum in range(self.seq_length):
            for node in range(numNodes):
